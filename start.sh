...
# and avoid binding conflicts on port 8189.
if ids="$(docker ps -q --filter "publish=8189")" && [[ -n "${ids}" ]]; then
  echo "发现占用 8189 的容器，正在清理..."
  docker rm -f ${ids}
fi
docker rm -f thymeleaf-test 2>/dev/null || true

docker build -t thymeleaf-test .
docker run -d --name thymeleaf-test -p 8189:8189 thymeleaf-test >/dev/null
echo "已启动: http://localhost:8189/playground"
